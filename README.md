...
CPU所做的工作都在软件层面，而GPU在硬件层面。大多数动画性能优化都是关于智能利用GPU和CPU,使得它们都不会超出负荷。

##### 动画的舞台
动画和屏幕上组合的图层实际上被一个单独的进程管理，而不是你的应用程序。这个进程就是渲染服务。在iOS5之前是SpringBoard进程，iOS6之后叫BacKBoard

当运行一段动画时候，这个过程会被四个分离的阶段

* 布局 - 这是准备你的视图/图层的层级关系，以及设置图层的属性(位置，背景色，别框)阶段
* 显示 - 这是图层的寄宿图被绘制的阶段，绘制有可能涉及你的 - drawRect: 和 - drawLayer：inContext的方法调用路径
* 准备 - 这时Core Animation准备发送动画数据到渲染服务阶段，这同时也是Core Animation将要执行一些别的事物例如解码动画过程将要显示的图片的时间点
* 提交 - 这是最后的阶段， Core Animation打包所有图层和动画属性，然后通过IPC发送到渲染服务进行显示

上述过程发送在你的应用程序之内，动画在屏幕上显示之前仍然有更多的工作，一旦打包的图层和动画到达渲染服务进程，他们会被反序列化形成另一个叫做渲染树的图层树，使用这个树状结构，渲染服务队动画的每一帧做出如下工作：
